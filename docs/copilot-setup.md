...
This option exposes all eight worker tools. Copilot can start (`worker_run`) and delete (`worker_cleanup`) worker sessions on the bridge host. `worker_wait` is a bounded long-poll (read-only, default 30s, clamped 1-120); drop it only for older v0.2.x bridges. Approval-gated work uses `worker_decide` plus `worker_resume`.
...
```json
{
  "mcpServers": {
    "opencode-bridge": {
      "type": "http",
      "url": "${COPILOT_MCP_BRIDGE_URL}",
      "tools": ["worker_catalog", "worker_run", "worker_wait", "worker_status", "worker_verify", "worker_cleanup", "worker_decide", "worker_resume"],
      "headers": {
        "Authorization": "Bearer ${COPILOT_MCP_BRIDGE_TOKEN}"
      }
    }
...
This bridge uses Bearer token authentication. It does not use OAuth in this path. It serves Streamable HTTP at `/mcp` (full catalog) and `/worker-mcp` (worker tools only: eight on this bridge; six on v0.3.0 bridges; five on older v0.2.x bridges).
...
14. Verify that the tool list shows the `worker_*` tools (eight on this bridge; six on v0.3.0 bridges; five on older v0.2.x bridges).
