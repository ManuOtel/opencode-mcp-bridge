...
`https://github.com/ManuOtel/opencode-mcp-bridge.git`, `ref=v0.4.4`). No
restructuring or plugin duplication was needed. The `v0.4.4` ref is the
...
version `0.4.4`). The Claude marketplace command installs from the
checked-out Git revision; the stable release is identified by Git tag
`v0.4.4` once the maintainer creates it.
