...
`https://github.com/ManuOtel/opencode-mcp-bridge.git`, `ref=v0.4.3`). No
restructuring or plugin duplication was needed. The `v0.4.3` ref is the
stable release tag; it exists only after the maintainer creates it.

```bash
codex plugin marketplace add ManuOtel/opencode-mcp-bridge --ref v0.4.3
```
...
version `0.4.3`). The Claude marketplace command installs from the
checked-out Git revision; the stable release is identified by Git tag
`v0.4.3` once the maintainer creates it.
